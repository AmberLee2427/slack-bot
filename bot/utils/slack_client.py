...
class SlackClient:
    ...
    async def send_message(
        self, channel: str, text: str = None, blocks: list = None, thread_ts: str = None
    ):
    ...
    async def update_message(
        ...
        """Update a message previously sent by the bot."""
        if not self.client:
            logger.error("Cannot update message - no Slack client")
            return None
    ...
    async def delete_message(self, channel: str, ts: str):
        """Delete a message previously sent by the bot."""
        if not self.client:
            logger.error("Cannot delete message - no Slack client")
            return None

        return await self.client.chat_delete(channel=channel, ts=ts)
