...
class MessageHandler:
    ...
    async def _handle_admin_rate_stats(self, channel: str, thread_ts: str = None):
        ...
        try:
            ...
            if all_stats["total_users"] == 0:
                stats_text = (
                    "📊 *Sonnet Allowance Statistics*\n\n"
                    f"Daily Sonnet Allowance: {all_stats['daily_limit']} queries per user\n"
                    f"Active Users: 0\n\n"
                ...
                stats_text += (
                    "\n_Sonnet allowances reset at midnight UTC; "
                    "the custom model handles additional queries._"
                )

            await self.slack_client.send_message(
                channel=channel, text=stats_text, thread_ts=thread_ts
            )
    ...
    async def _handle_personal_stats(
        self, channel: str, user_id: str, thread_ts: str = None
    ):
        ...
        try:
            ...
            stats_text = (
                ...
                f"_Your Sonnet allowance resets at midnight UTC. "
                f"Nancy remains available through the custom model._"
            )

            # Add helpful tips based on usage
            if percentage >= 90:
                stats_text += (
                    "\n\n💡 _Sonnet is running low; Nancy will automatically "
                    "continue on the custom model._"
                )
            elif percentage >= 70:
