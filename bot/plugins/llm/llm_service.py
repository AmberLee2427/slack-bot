...
)  # Default 100 queries per user per day
...
class LLMService:
    def __init__(
        ...
        self.custom_api_key = os.environ.get("CUSTOM_API_KEY", CUSTOM_API_KEY)
        self.custom_model = os.environ.get("CUSTOM_MODEL", CUSTOM_MODEL)
        self.custom_url = os.environ.get("CUSTOM_URL", CUSTOM_URL or "").rstrip("/")
        ...
        if not MCP_BASE_URL:
            logger.error(
                "MCP_BASE_URL is not set. MCPRAGAdapter is now required as the only supported RAG backend. Exiting."
            )
            raise RuntimeError(
                "MCP_BASE_URL is required for Nancy. No legacy RAGService fallback is available."
            )
        ...
        try:
            ...
            if not resp.ok:
                logger.error(
                    "MCP RAG health check failed: %s: %s", resp.status_code, resp.text
                )
                raise RuntimeError(
                    f"MCPRAGAdapter health check failed: {resp.status_code} {resp.text}"
                )
            self.rag_status = {"available": True, "source": "mcp"}
            logger.info("Using MCPRAGAdapter pointing to %s (health OK)", MCP_BASE_URL)
        except Exception as e:
            logger.error(
                "Failed to initialize MCPRAGAdapter or health-check (%s): %s",
                MCP_BASE_URL,
                e,
            )
            ...
            if self.thread_cache_path.exists():
                with open(self.thread_cache_path, "r") as f:
                    ...
                    if self.debugging:
                        ...
                        logger.info(
                            f"Loaded thread cache with {len(thread_cache)} threads"
                        )
                    return thread_cache
        except Exception as e:
            logger = __import__("logging").getLogger(__name__)
    ...
    def _save_thread_cache(self):
        ...
        try:
            ...
            if self.debugging:
                logger = __import__("logging").getLogger(__name__)
                logger.info(
                    f"Saved thread cache with {len(self.thread_retrievals)} threads"
                )
        except Exception as e:
            logger = __import__("logging").getLogger(__name__)
    ...
    def _prune_thread_cache(self, max_threads: int = 50):
        ...
        if len(self.thread_retrievals) > max_threads:
            ...
            sorted_threads = sorted(
                self.thread_retrievals.items(), key=lambda x: x[1][0], reverse=True
            )
            ...
            if self.debugging:
                logger = __import__("logging").getLogger(__name__)
                logger.info(
                    f"Pruned thread cache to {len(self.thread_retrievals)} threads"
                )
    ...
    def update_rag_variables(self):
        ...
        try:
            all_indexed_docs = list(
                self.rag.embeddings.database.search("select id, text from txtai")
            )
            self.all_indexed_files = [
                doc["id"] for doc in all_indexed_docs if doc.get("id")
            ]
            self.indexed_file_map = {
                doc["id"]: doc.get("text", "")
            ...
            logger = __import__("logging").getLogger(__name__)
            logger.warning(
                f"RAG warmup (embeddings SQL) failed; continuing without file index: {e}"
            )
            self.all_indexed_files = []
            self.indexed_file_map = {}
    ...
    def get_initial_context(self, query: str, thread_ts: str = None) -> str:
        ...
        if thread_ts and thread_ts in self.thread_retrievals:
            ...
            if cached_doc_ids:
                logger = __import__("logging").getLogger(__name__)
                logger.info(
                    f"Using cached thread context: {len(cached_doc_ids)} files from thread {thread_ts}"
                )
    ...
    def construct_query_payload(
        ...
        return {"system": self.system_prompt, "messages": messages}

    def _provider_for_interaction(
        ...
        if not allowed:
            if self.custom_enabled:
                logging.getLogger(__name__).info(
                    "User %s exhausted the Sonnet allowance; using custom model %s",
                    user_id,
                    self.custom_model,
                )
                return "custom", None
        ...
        if remaining == 0:
            logger.warning(
                "User %s has reached their daily Sonnet limit: %s/%s",
                user_id,
                used_today,
            ...
            logger.info(
                "User %s has %s Sonnet queries remaining today", user_id, remaining
            )
        else:
            logger.info(
                "User %s Sonnet allowance: %s/%s used",
                user_id,
                used_today,
                used_today + remaining,
            )
    ...
    def _query_custom(self, payload: dict) -> str:
        """Query the configured OpenAI-compatible fallback."""
        ...
        if not isinstance(content, str) or not content.strip():
            raise ValueError("custom LLM returned an empty or non-text response")
        return content

    @staticmethod
    def _should_fallback_from_anthropic(exc: Exception) -> bool:
        """Return true for quota, overload, and transient provider failures."""
        status_code = getattr(exc, "status_code", None)
        if status_code in {402, 408, 429, 500, 502, 503, 504, 529}:
            return True
        ...
        try:
            ...
            if active_provider == "custom":
                ...
                if anthropic_client is None:
                    ...
                    logger.warning("Anthropic is not configured; using custom fallback")
                    ...
                    try:
                        ...
                        llm_text = "".join(text_blocks)
                    except Exception as exc:
                        if (
                            not self.custom_enabled
                            or not self._should_fallback_from_anthropic(exc)
                        ):
                            raise
                        logger.warning(
                            "Anthropic unavailable for turn %s; falling back to %s: %s",
                            turn,
                            self.custom_model,
                            exc,
                        )
            ...
            if llm_text and llm_text.strip():
                ...
                logger.info(
                    "%s API response received for turn %s", active_provider, turn
                )
                assistant_response = f"\n\nTURN {turn+1}:\n\n"
                assistant_response += llm_text + "\n"
                return llm_text, assistant_response
            else:
                logger.error("Empty response from %s API", active_provider)
                return None, None
    ...
    def continue_with_extended_turns(
        self,
        callback_fn,
        ...
        logger.info(
            f"🍳 Keep Cooking: Extending analysis with {additional_turns} additional turns"
        )
    ...
    def call_llm_with_callback(
        ...
        if conversation_history:
            ...
            if self.debugging:
                for i, msg in enumerate(conversation_history):
                    ...
                    is_bot = msg.get("is_bot", False)
                    logger.info(
                        f"  Message {i+1}: {'[BOT] ' if is_bot else ''}#{user}: {text}..."
                    )
        else:
        ...
        provider, rate_limit_msg = self._provider_for_interaction(user_id)
        if rate_limit_msg:
            callback_fn(rate_limit_msg, True, hit_turn_limit=False)
            return
        ...
        while searching and turn < self.max_turns:
            ...
            # --- QUERY LLM ---
            llm_text, assistant_response = self.querry_llm(
                payload,
                turn,
                user_id,
                provider=provider,
            ...
            if llm_text is None:
                ...
                callback_fn(
                    "  :x: _Failed to connect to LLM Service_",
                    True,
            ...
            if "🚫 *Daily Limit Reached*" in llm_text:
                ...
                callback_fn(
                    llm_text, True, hit_turn_limit=False
                )  # Send rate limit message as final response
                return
            # --- END QUERY LLM ---
            ...
            if "RETRIEVE:" in llm_text:
                ...
                for file_path in file_paths:
                    ...
                    if github_url:
                        ...
                        callback_fn(
                            f"  :page_facing_up: _Retrieving file:_ {display_link}",
                            False,
                        )
                    else:
                        callback_fn(
                            f"  :page_facing_up: _Retrieving file:_ `{file_path}`",
                            False,
                        )
            ...
            if "WEIGHT:" in llm_text:
                ...
                self.model_weights, meta_prompt_addition = self.weight_tool(
                    self, llm_text, self.model_weights, self.model_weights_path
            ...
            if "TREE:" in llm_text:
                callback_fn(
                    "  :deciduous_tree: _Exploring knowledge base structure_", False
                )
            ...
            if "RESPONSE" in llm_text:
                ...
                try:
                    ...
                    if response_text:
                        ...
                        if self.debugging:
                            logger.warning(
                                "response_tool returned None despite RESPONSE being in text"
                            )
                except Exception as e:
                    logger.error(
                        f"Error in response extraction/callback: {e}", exc_info=True
                    )
            else:
                if self.debugging:
