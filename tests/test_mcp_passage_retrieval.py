...
def test_retrieve_endpoint_returns_passage(mcp_server):
    ...
    search_resp = requests.get(f"{MCP_BASE_URL.rstrip('/')}/search", params={"query": "roman", "limit": 1}, headers=headers, timeout=30)
    ...
    resp = requests.post(f"{MCP_BASE_URL.rstrip('/')}/retrieve", json=payload, headers=headers, timeout=30)
